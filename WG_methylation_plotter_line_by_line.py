...
parser.add_argument('-binfile', dest='file1',action="store", type=argparse.FileType('r'),help="tab delimited bin file see read_bins()")
parser.add_argument('-metfile', dest='file2',action="store", type=argparse.FileType('r'),help="tab delimited methylation file from BSmap, see split_metfile()")
parser.add_argument('-p', dest='cpu',nargs='?',default=1,action="store",type=int,help="# of processes, cpu cores you would like to run on")  # ? is for positional arguments that may or maynot exist default is only 1 process

args = parser.parse_args()

cwd = os.getcwd()
genels = str(args.file2.name.strip())
chr1_met = (cwd + "/"+ genels[:-4] + "_WGplot_chr1.txt")
chr2_met = (cwd + "/"+ genels[:-4] + "_WGplot_chr2.txt")
chr3_met = (cwd + "/"+ genels[:-4] + "_WGplot_chr3.txt")
chr4_met = (cwd + "/"+ genels[:-4] + "_WGplot_chr4.txt")
chr5_met = (cwd + "/"+ genels[:-4] + "_WGplot_chr5.txt")
...
#chr1	10	+	CHH	1.000	2.00	2	2	4	4	0.342	1.000
    ...
    with args.file2 as source_file:
        next(source_file)
